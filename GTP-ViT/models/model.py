...
"""
1. Different backbone: LV-VIT-S, LV-VIT-M
2. Different size: ViT-S, ViT-B, ViT-L
3. Different training approach: ViT-B-AugReg, ViT-B-CLIP, ViT-B-DINO, ViT-B-MAE
4. Different token numbers: ViT-S-Patch8, ViT-B-Patch8
5. Without CLS token: RPN, GAP, EVA
"""
...
def propagate(x: torch.Tensor, weight: torch.Tensor,
    ...
    if standard == "None":
        ...
        if cls_token:
            ...
            weight_prop = weight.gather(dim=2, index=index_prop.unsqueeze(1).expand(-1, weight.shape[1],
                                                                                    -1))  # B, N-1-num_prop, num_prop
            weight = weight.gather(dim=2, index=index_kept.unsqueeze(1).expand(-1, weight.shape[1],
                                                                               -1))  # B, N-1-num_prop, N-1-num_prop
        else:
...
def select(weight: torch.Tensor, standard: str = "None", num_prop: int = 0, cls_token=True):
    ...
    assert len(
        weight.shape) == 4, "Selection methods on tensors other than the attention map haven't been supported yet."
...
class Attention(nn.Module):
    ...
    def forward(self, x, token_scales=None):
        ...
        if self.sparsity < 1:
            ...
            threshold = torch.kthvalue(attn.reshape(B, self.num_heads, -1), k, dim=-1, keepdim=True)[0].unsqueeze(
                -1)  # B,H,1,1
            if self.training:
...
@register_model
def graph_propagation_vit_base_patch8_224_augreg(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                                                 **kwargs):
...
@register_model
def graph_propagation_vit_base_patch16_224_augreg(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                                                  **kwargs):
    model = GraphPropagationTransformer(patch_size=16, embed_dim=768, depth=12,
                                        num_heads=12, mlp_ratio=4, qkv_bias=True,
                                        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
...
@register_model
def graph_propagation_vit_base_patch16_224_clip(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                                                **kwargs):
    model = GraphPropagationTransformer(patch_size=16, embed_dim=768, depth=12,
...
@register_model
def graph_propagation_vit_base_patch16_224_mae(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                                               **kwargs):
    model = GraphPropagationTransformer(patch_size=16, embed_dim=768, depth=12,
                                        num_heads=12, mlp_ratio=4, qkv_bias=True, pre_norm=True,
                                        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
...
@register_model
def graph_propagation_vit_base_patch16_224_dino(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                                                **kwargs):
    model = GraphPropagationTransformer(patch_size=16, embed_dim=768, depth=12,
                                        num_heads=12, mlp_ratio=4, qkv_bias=True,
                                        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
...
def tome_vit_base_patch16_224_augreg(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None, **kwargs):
    model = VisionTransformer(patch_size=16, embed_dim=768, depth=12,
                              num_heads=12, mlp_ratio=4, qkv_bias=True,
                              norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)  # , **kwargs)
...
#     net = tome_vit_large_patch16_224_augreg(num_classes=5)
#     summary(net, input_size=(1, 3, 224, 224))

    """
    set: model.r = 0
    ==========================================================================================
    Total params: 303,306,757
    Trainable params: 303,306,757
    Non-trainable params: 0
    Total mult-adds (M): 456.66
    ==========================================================================================
    Input size (MB): 0.60
    Forward/backward pass size (MB): 429.27
    Params size (MB): 1212.42
    Estimated Total Size (MB): 1642.29
    ==========================================================================================
    """
    ...
    ==========================================================================================
    Input size (MB): 0.60
    Forward/backward pass size (MB): 324.28
    Params size (MB): 1212.42
    Estimated Total Size (MB): 1537.30
    ==========================================================================================`
    """